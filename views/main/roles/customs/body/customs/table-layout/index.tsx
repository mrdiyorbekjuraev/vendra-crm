	...
	return (
		<div className="ag-theme-alpine border-b border-t h-[calc(100%-205px)]">
			...
			{Boolean(selectedNodes.length) && (
				<div className="absolute bottom-15 shadow-xl left-1/2 transform -translate-x-1/2 max-w-[660px] w-full h-12 border rounded-xl flex items-center px-4 bg-white dark:bg-[#1C1D21] gap-2">
					...
					<Button
						...
						onClick={() =>
							setModal({
								editRole: { open: true, props: selectedNodes?.[0]?.data },
							})
						}
					>
						<Pen />
								...
								onClick={() =>
									setModal({
										remove: {
											open: true,
											props: {
												roleIds: selectedNodes?.map((el) => el?.data?.id),
											},
										},
									})
								}
							>
								<Trash2Icon /> Delete role(s)
							</DropdownMenuItem>
						</DropdownMenuContent>
					</DropdownMenu>
					<Button
